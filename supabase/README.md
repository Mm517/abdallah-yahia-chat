...
- فعّل Email.
- عطّل تأكيد البريد الإلكتروني (Confirm email) — التسجيل يجب أن يسجّل الدخول
  مباشرة بخطوة واحدة، بدون أي تحقق ثنائي.
- عطّل Google وأي OAuth provider آخر؛ الهدف لا يستخدم Google Login.
- عطّل Phone وMFA/TOTP — غير مستخدمين في هذا المشروع.

تسجيل المستخدم المقترح:

1. الواجهة تستدعي `supabase.auth.signUp({ email, password, options: { data: { display_name } } })`.
2. لأن Confirm email متعطّل، Supabase يرجّع session فورًا مع رد signUp نفسه.
3. trigger `handle_new_user()` ينشئ صف `public.profiles`.
4. الاستعادة تستخدم `resetPasswordForEmail`؛ لا تحفظ password أو refresh token في `public`.
