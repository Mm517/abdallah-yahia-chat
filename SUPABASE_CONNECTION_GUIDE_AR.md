...
- فعّل Email.
- عطّل Email Confirmation (Confirm email) — المشروع مطلوب منه تسجيل دخول
  مباشر بخطوة واحدة بدون أي تحقق ثنائي/تأكيد بريد أو MFA.
- عطّل Google وأي OAuth provider آخر حسب متطلبات المشروع.
- عطّل Phone وMFA/TOTP — غير مطلوبين في هذا المشروع.

المسار المستهدف للتسجيل:

1. `supabase.auth.signUp({ email, password })`
2. trigger ينشئ `public.profiles`.
3. تسجيل الدخول يتم فورًا (session بترجع من `signUp` نفسها لأن Confirm email متعطّل).
4. استخدام `resetPasswordForEmail` لاستعادة كلمة المرور.
...
- التسجيل (دخول مباشر بدون تأكيد بريد) وتسجيل الدخول والخروج.
- استعادة كلمة المرور.
